(function () {
  ...
  function render() {
    ...
    $('disputeShift').innerHTML = shifts.filter(s=>String(s.user_id)===String(state.current_user_id)).map(s => `<option value="${esc(s.shift_id)}">${esc(s.store_name)} · ${esc(localTime(s.clock_in_at))}</option>`).join('');
    const allStores=state.stores||[],storeOptions=allStores.map(s => `<option value="${Number(s.id)}">${esc(s.store_name)}</option>`).join('');
    const myWorking=new Set(working.filter(person=>String(person.user_id)===String(state.current_user_id)).map(person=>person.store_name));
    $('financialStore').innerHTML = state.is_super?storeOptions:allStores.filter(s=>myWorking.has(s.store_name)).map(s=>`<option value="${Number(s.id)}">${esc(s.store_name)}</option>`).join('');
    $('proposedStore').innerHTML=storeOptions;
  ...
  function updateDisputeForm(){const type=$('disputeType').value,isNew=type==='new_shift';$('proposedStoreField').hidden=!isNew;$('disputeShiftField').hidden=isNew;$('disputeShift').disabled=isNew;
  ...
  $('disputeType').addEventListener('change',updateDisputeForm);updateDisputeForm();
