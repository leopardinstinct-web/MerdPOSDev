...
A DEV-only **Current role** presentation selector may preview Admin, Super or User rendering from the current client role/LOA policy, but it MUST NOT mutate the authenticated DEV session, backend/API authorization or database role. DEV identity remains authoritative for Studio availability.
