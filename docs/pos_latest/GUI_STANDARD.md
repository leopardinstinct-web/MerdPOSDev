...
UI Studio is an actual-DEV-only design tool, not product chrome. Its yellow/amber hub is visible by default for DEV and uses one icon-only radial **sector ring** at a time. The Studio17 root begins with Minimize, Select, Edit Dashboard and Settings; with one selection it also exposes Add, Edit, Move, Comment and Hide/Show, while Undo remains available on every level. Minimize is desktop-only and restores from a circular control immediately to the right of the account circle. The center hub is the primary action display: hover opens the ring on fine-pointer devices, wheel input over the hub cycles available actions, clicking executes the armed action, and an unarmed click becomes Back inside nested layers. Bright high-chroma Studio accents may be used for tool-layer differentiation; they are not MERDPOS brand tokens and MUST NOT leak into normal portal components.

A DEV-only **Current role** selector offers Developer, Admin, Super or User. Developer restores the actual DEV website view; Admin/Super/User preview universally from the current client role/LOA policy. The authenticated DEV identity and database role remain unchanged, but normal portal rendering, navigation, dashboard layout/data, API permission checks and feature actions MUST use the selected role as the effective website role. Actual-DEV exceptions are limited to DEV tooling such as UI Studio and Working client switching.

Studio must preserve the rendered application as the canvas. Selection mode may target navigation, bottom-dock/contextual navigation and transient utility controls, but must intercept the application action while selecting. Studio interactions must not close an opened shell utility menu. Comments, widget descriptions, added preview elements, Studio accent/size settings and change history remain local preview/design metadata. Edit Dashboard is the explicit exception: it delegates to the existing persistent Dashboard Builder and its dashboard-layout API, actual-DEV gated when launched from Studio; ordinary Studio patches remain no-API/no-DB/no-source-write.
