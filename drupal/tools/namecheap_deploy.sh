...
PARITY_PROBE="$(php84 "$DRUSH_PHP" --root="$WEB" php:eval \
  '$r=\Drupal::service("merdpos_core.parity_provider"); $g=\Drupal::service("merdpos_core.portal_gateway")->call("beta_state","GET"); $gp=$g["payload"]??[]; $shop=!empty($gp["current_user"]["shop_active"]); $s=["home"=>$r->home(),"operations"=>$r->section("operations"),"reports"=>$r->section("reports"),"finance"=>$r->section("finance"),"dev"=>$r->section("dev")]; $o=array_map(static fn($v)=>$v["status"]??null,$s); $o["shop_active"]=$shop; $o["finance_available"]=!empty($gp["current_user"]["finance_available"]); echo json_encode($o,JSON_UNESCAPED_SLASHES);')"
php84 -r '$p=json_decode($argv[1],true); if(!is_array($p)){fwrite(STDERR,"Five-surface parity self-test failed.\n");exit(1);} foreach(["home","operations","reports","dev"] as $key){if(($p[$key]??"")!=="ok"){fwrite(STDERR,"Five-surface parity self-test failed at {$key}.\n");exit(1);}} $shop=!empty($p["shop_active"]); $finance=$p["finance"]??""; if(($shop&&$finance!=="ok")||(!$shop&&$finance!=="forbidden")||(!$shop&&!empty($p["finance_available"]))){fwrite(STDERR,"Finance Shop-login parity self-test failed.\n");exit(1);}' "$PARITY_PROBE"
...
FINANCE_V2_PROBE="$(php84 "$DRUSH_PHP" --root="$WEB" php:eval \
  '$r=\Drupal::service("merdpos_core.parity_provider")->section("finance",[]); $g=\Drupal::service("merdpos_core.portal_gateway")->call("beta_state","GET"); $gp=$g["payload"]??[]; $perms=$gp["permissions"]??[]; $shop=!empty($gp["current_user"]["shop_active"]); $route=\Drupal::service("router.route_provider")->getRouteByName("merdpos_core.finance"); echo json_encode(["status"=>$r["status"]??null,"shop_active"=>$shop,"finance_available"=>!empty($gp["current_user"]["finance_available"]),"finance_view"=>!empty($perms["finance.view"]),"finance_submit"=>!empty($perms["finance.submit"]),"finance_open_day"=>!empty($perms["finance.open_day"]),"route"=>$route->getPath()],JSON_UNESCAPED_SLASHES);')"
php84 -r '$p=json_decode($argv[1],true); if(!is_array($p)||empty($p["finance_view"])||empty($p["finance_submit"])||empty($p["finance_open_day"])||($p["route"]??"")!=="/merdpos/finance"){fwrite(STDERR,"Finance v2 self-test failed.\n");exit(1);} $shop=!empty($p["shop_active"]); if(($shop&&($p["status"]??"")!=="ok")||(!$shop&&($p["status"]??"")!=="forbidden")||(!$shop&&!empty($p["finance_available"]))){fwrite(STDERR,"Finance v2 Shop-login guard self-test failed.\n");exit(1);}' "$FINANCE_V2_PROBE"
