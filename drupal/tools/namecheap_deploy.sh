...
php84 -r '$p=json_decode($argv[1],true); if(!is_array($p)||($p["status"]??"")!=="ok"||($p["role"]??"")!=="DEV"||($p["loa"]??0)!==1000||($p["filters"]??-1)!==0||($p["metrics"]??0)!==2||($p["charts"]??-1)!==0||($p["week_options"]??0)<1||($p["groups"]??0)!==4||($p["export_columns"]??0)<7||empty($p["payroll"])){fwrite(STDERR,"Reports v2 self-test failed.\n");exit(1);}' "$REPORTS_V2_PROBE"
