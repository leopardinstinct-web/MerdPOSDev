...
php84 "$DRUPAL/tools/validate_global_page_headers_v1.php"
php84 "$DRUPAL/tools/validate_global_ui_primitives_v1.php"
...
php84 "$DRUPAL/tools/validate_security_headers_v1.php"
...
PARITY_PROBE="$(php84 "$DRUSH_PHP" --root="$WEB" php:eval \
  '$r=\Drupal::service("merdpos_core.parity_provider"); $gw=\Drupal::service("merdpos_core.portal_gateway"); $g=$gw->call("beta_state","GET"); $gp=$g["payload"]??[]; $shop=!empty($gp["current_user"]["shop_active"]); $s=["home"=>$r->home(),"operations"=>$r->section("operations"),"reports"=>$r->section("reports"),"finance"=>$r->section("finance"),"dev"=>$r->section("dev")]; $o=array_map(static fn($v)=>$v["status"]??null,$s); $o["finance_surface_status"]=$o["finance"]??null; $o["shop_active"]=$shop; $o["finance_available"]=!empty($gp["current_user"]["finance_available"]); $fg=$shop?["status"=>"not_checked","payload"=>[]]:$gw->call("financials","GET"); $fgp=$fg["payload"]??[]; $o["finance_guard_status"]=$fg["status"]??null; $o["finance_guard_success"]=$fgp["success"]??null; $o["finance_guard_error_code"]=$fgp["error_code"]??null; if(!$shop && ($o["finance_guard_status"]??"")==="ok" && ($o["finance_guard_success"]??true)===false && ($o["finance_guard_error_code"]??"")==="shop_login_required" && empty($o["finance_available"])) $o["finance"]="ok"; echo json_encode($o,JSON_UNESCAPED_SLASHES);')"
php84 -r '$p=json_decode($argv[1],true); if(!is_array($p)){fwrite(STDERR,"Five-surface parity self-test failed.\n");exit(1);} foreach(["home","operations","reports","finance","dev"] as $key){if(($p[$key]??"")!=="ok"){fwrite(STDERR,"Five-surface parity self-test failed at {$key}.\n");exit(1);}} $shop=!empty($p["shop_active"]); if(!$shop && (($p["finance_guard_status"]??"")!=="ok" || ($p["finance_guard_success"]??true)!==false || ($p["finance_guard_error_code"]??"")!=="shop_login_required" || !empty($p["finance_available"]))){fwrite(STDERR,"Finance Shop-login guard self-test failed.\n");exit(1);}' "$PARITY_PROBE"
...
REPORTS_V2_PROBE="$(php84 "$DRUSH_PHP" --root="$WEB" php:eval \
  '$r=\Drupal::service("merdpos_core.parity_provider")->section("reports",[]); $role=$r["role"]??[]; echo json_encode(["status"=>$r["status"]??null,"role"=>$role["key"]??null,"loa"=>$role["loa"]??null,"filters"=>count($r["filters"]??[]),"metrics"=>count($r["metrics"]??[]),"charts"=>count($r["chart_specs"]??[]),"week_options"=>count($r["week_options"]??[]),"groups"=>count($r["groups"]??[]),"export_columns"=>count($r["export_columns"]??[]),"export_rows"=>count($r["export_rows"]??[]),"payroll"=>!empty($r["payroll_visible"])],JSON_UNESCAPED_SLASHES);')"
php84 -r '$p=json_decode($argv[1],true); if(!is_array($p)||($p["status"]??"")!=="ok"||($p["role"]??"")!=="DEV"||($p["loa"]??0)!==1000||($p["filters"]??-1)!==0||($p["metrics"]??0)!==2||($p["charts"]??-1)!==0||($p["week_options"]??0)<1||($p["groups"]??0)!==3||($p["export_columns"]??0)<7||empty($p["payroll"])){fwrite(STDERR,"Reports v2 self-test failed.\n");exit(1);}' "$REPORTS_V2_PROBE"


FINANCE_V2_PROBE="$(php84 "$DRUSH_PHP" --root="$WEB" php:eval \
  '$r=\Drupal::service("merdpos_core.parity_provider")->section("finance",[]); $gw=\Drupal::service("merdpos_core.portal_gateway"); $g=$gw->call("beta_state","GET"); $gp=$g["payload"]??[]; $perms=$gp["permissions"]??[]; $shop=!empty($gp["current_user"]["shop_active"]); $fg=$shop?["status"=>"not_checked","payload"=>[]]:$gw->call("financials","GET"); $fgp=$fg["payload"]??[]; $route=\Drupal::service("router.route_provider")->getRouteByName("merdpos_core.finance"); echo json_encode(["status"=>$r["status"]??null,"guard_status"=>$fg["status"]??null,"guard_success"=>$fgp["success"]??null,"guard_error_code"=>$fgp["error_code"]??null,"shop_active"=>$shop,"finance_available"=>!empty($gp["current_user"]["finance_available"]),"finance_view"=>!empty($perms["finance.view"]),"finance_submit"=>!empty($perms["finance.submit"]),"finance_open_day"=>!empty($perms["finance.open_day"]),"route"=>$route->getPath()],JSON_UNESCAPED_SLASHES);')"
php84 -r '$p=json_decode($argv[1],true); if(!is_array($p)||empty($p["finance_view"])||empty($p["finance_submit"])||empty($p["finance_open_day"])||($p["route"]??"")!=="/merdpos/finance"){fwrite(STDERR,"Finance v2 self-test failed.\n");exit(1);} $shop=!empty($p["shop_active"]); if(($shop&&($p["status"]??"")!=="ok")||(!$shop&&(($p["guard_status"]??"")!=="ok" || ($p["guard_success"]??true)!==false || ($p["guard_error_code"]??"")!=="shop_login_required"))||(!$shop&&!empty($p["finance_available"]))){fwrite(STDERR,"Finance v2 Shop-login guard self-test failed.\n");exit(1);}' "$FINANCE_V2_PROBE"
...
ADMIN_V1_PROBE="$(php84 "$DRUSH_PHP" --root="$WEB" php:eval \
  '$g=\Drupal::service("merdpos_core.portal_gateway"); $c=$g->call("clients","GET"); $x=$g->call("client_context","GET"); $cp=$c["payload"]??[]; $xp=$x["payload"]??[]; $home=(int)($xp["home_client_id"]??0); $target=$home; foreach(($xp["clients"]??[]) as $row){$id=(int)($row["id"]??0); if($id>0&&$id!==$home){$target=$id;break;}} $d=$g->call("admin_directory","GET",[],[],$target>0?$target:null); $dp=$d["payload"]??[]; $v=$target>0?$g->call("store_devices","GET",[],[],$target):["status"=>"invalid","payload"=>[]]; $vp=$v["payload"]??[]; echo json_encode(["clients_status"=>$c["status"]??null,"clients"=>count($cp["clients"]??[]),"directory_status"=>$d["status"]??null,"devices_status"=>$v["status"]??null,"devices_is_array"=>is_array($vp["devices"]??null),"target_client_id"=>$target,"active_client_id"=>$dp["active_client_id"]??null,"stores_manage"=>!empty($dp["permissions"]["stores.manage"]),"stores_devices_manage"=>!empty($dp["permissions"]["stores.devices.manage"]),"workforce_manage"=>!empty($dp["permissions"]["workforce.manage"]),"stores"=>count($dp["stores"]??[]),"employees"=>count($dp["employees"]??[])],JSON_UNESCAPED_SLASHES);')"
php84 -r '$p=json_decode($argv[1],true); if(!is_array($p)||($p["clients_status"]??"")!=="ok"||($p["directory_status"]??"")!=="ok"||($p["devices_status"]??"")!=="ok"||empty($p["devices_is_array"])||($p["clients"]??0)<1||empty($p["stores_manage"])||empty($p["stores_devices_manage"])||empty($p["workforce_manage"])||($p["target_client_id"]??0)!==($p["active_client_id"]??-1)){fwrite(STDERR,"Administration v1 signed read/context self-test failed.\n");exit(1);}' "$ADMIN_V1_PROBE"
...
ATTENDANCE_QR_V1_PROBE="$(php84 "$DRUSH_PHP" --root="$WEB" php:eval \
  '$p=\Drupal::service("merdpos_core.parity_provider")->home([]); $g=\Drupal::service("merdpos_core.portal_gateway")->call("attendance_scan","POST",[],["token"=>"x.x"]); $gp=$g["payload"]??[]; $route=\Drupal::service("router.route_provider")->getRouteByName("merdpos_core.attendance_scan"); echo json_encode(["status"=>!empty($p["can_scan_shop"])?"ok":"failed","shop_active"=>!empty($p["shop_active"]),"route"=>$route->getPath(),"invalid_probe_status"=>$g["status"]??null,"invalid_probe_http"=>$g["http_status"]??null,"invalid_probe_success"=>$gp["success"]??null,"invalid_probe_error_code"=>$gp["error_code"]??null],JSON_UNESCAPED_SLASHES);')"
php84 -r '$p=json_decode($argv[1],true); $h=(int)($p["invalid_probe_http"]??0); if(!is_array($p)||($p["status"]??"")!=="ok"||($p["route"]??"")!=="/merdpos/attendance/scan"||($p["invalid_probe_status"]??"")!=="ok"||$h!==200||($p["invalid_probe_success"]??true)!==false||($p["invalid_probe_error_code"]??"")!=="invalid_qr"){fwrite(STDERR,"Home Shop QR v1 self-test failed.\n");exit(1);}' "$ATTENDANCE_QR_V1_PROBE"
