...
final class ParityDataProvider implements ParityDataProviderInterface {
  ...
  public function home(array $query = []): array {
    ...
    $stateRoleKey = strtoupper((string)($statePayload['role_key'] ?? $statePayload['role'] ?? 'USER'));
    $canScanAttendance = !empty($permissionMap['attendance.scan']) || in_array('attendance.scan', $permissions, true);
    $canViewFinance = !empty($permissionMap['finance.view']) || in_array('finance.view', $permissions, true);
    $isActualDev = !empty($statePayload['is_dev']);
    $canScanShop = $stateRoleKey === 'USER' && !$isActualDev ? $canScanAttendance : $canViewFinance;
    $shopState = $this->map($statePayload['current_user'] ?? []);
    ...
    $surface['can_scan_shop'] = $canScanShop;
    $surface['shop_active'] = !empty($shopState['shop_active']);
    $surface['shop_name'] = (string)($shopState['shop_name'] ?? '');
    $surface['shop_mode'] = (string)($shopState['shop_mode'] ?? '');
    $surface['finance_available'] = !empty($shopState['finance_available']);
