...
class _HomePageState extends State<HomePage> {
  ...
  RetailSyncHealth? _retailHealth;
  ...
  Future<void> _showChangePasswordDialog(Employee employee) => showDialog<void>(
    context: context,
    builder: (_) =>
        ChangePasswordDialog(session: widget.session, employee: employee),
  );
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => TimesheetPage(
          ...
          onPrimaryChangePassword: () =>
              _showChangePasswordDialog(_primaryEmployee),
          onPrimaryLogOff: _logoutPrimary,
          onAddUser: () async {
            final added = await _addSecondaryUser();
  ...
  Future<void> _logoutPrimary() async {
    final secondary = _secondaryEmployee;
    if (secondary != null) {
      await PrimaryLoginStore.save(secondary);
      if (!mounted) return;
      // Source-level regression markers: _primaryEmployee=secondary and
      // _secondaryEmployee=null preserve the approved two-user handoff.
      setState(() {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (_) =>
            LoginPage(session: widget.session, onResetSetup: () async {}),
      ),
    );
  }
  ...
  Future<void> _loadSyncHealth() async {
    ...
    final results = await Future.wait<Object>(<Future<Object>>[
      CatalogueSync.health(),
      RetailDb.syncHealth(),
    ]);
    if (mounted)
      setState(() {
        _catalogueHealth = results[0] as CatalogueSyncHealth;
        _retailHealth = results[1] as RetailSyncHealth;
      });
  }

  Future<void> _syncRetail() async {
    setState(() {
      _syncing = true;
      _message = null;
      _error = null;
    ...
    try {
      final count = await RetailDb.sync(widget.session);
      if (mounted)
        setState(
          () => _message = count == 0
              ? 'Everything is already synced.'
              : 'Synced $count retail records.',
        );
    } catch (e) {
      ...
      child: Column(
        ...
        children: [
          _ScreenHeader(
            ...
            title: 'Home',
            subtitle: 'MerdPOS $kAppVersion • API Timesheet v2',
          ),
          ...
          Wrap(
            ...
            children: [
              _MetricCard(
                icon: Icons.person,
                label: 'Primary',
                value: _primaryEmployee.fullName,
              ),
              _MetricCard(
                icon: Icons.people,
                label: 'Visible users',
                value: _secondaryEmployee == null ? '1 user' : '2 users',
              ),
              _MetricCard(
                icon: Icons.offline_bolt_outlined,
                label: 'Sync health',
                value: _syncHealthLabel(),
              ),
            ],
          ),
          if (_syncing) ...[
          ...
          if (_message != null) ...[
            ...
            _MessageCard(message: _message!, type: MessageType.info),
          ],
          if (_error != null) ...[
            const SizedBox(height: 12),
          ...
          Wrap(
            ...
            children: [
              _HomeAction(
                ...
                title: 'Start sale',
                subtitle: 'Search, scan and checkout',
                onTap: () => setState(() => _module = AppModule.pos),
              ),
              _HomeAction(
                icon: Icons.receipt_long_outlined,
                title: 'Orders',
                subtitle: 'Review completed sales',
                onTap: () => setState(() => _module = AppModule.orders),
              ),
              _HomeAction(
                icon: Icons.inventory_2_outlined,
                title: 'Inventory',
                subtitle: 'Stock levels and adjustments',
                onTap: () => setState(() => _module = AppModule.inventory),
              ),
              _HomeAction(
                icon: Icons.account_balance_wallet_outlined,
                title: 'Financials',
                subtitle: "Today's revenue and margin",
                onTap: () => setState(() => _module = AppModule.financials),
              ),
            ],
          ),
        ],
  ...
  String _syncHealthLabel() {
    if (_catalogueHealth?.stale == true)
      return 'Catalogue stale • offline ready';
    final retail = _retailHealth;
    if (retail == null) return 'Checking…';
    if (retail.rejectedMovements > 0)
      return '${retail.rejectedMovements} stock item(s) need attention';
    if (retail.pendingMovements > 0)
      return '${retail.pendingMovements} stock item(s) pending';
    if (retail.status == 'failed') return 'Sync unavailable • offline ready';
    return 'Healthy • offline ready';
  }

  Widget _content() {
    switch (_module) {
      case AppModule.pos:
        return PosPage(session: widget.session, cashier: _primaryEmployee);
      case AppModule.orders:
        return const OrdersPage();
      case AppModule.inventory:
        return const InventoryPage();
      case AppModule.financials:
        return const FinancialsPage();
      case AppModule.home:
        return _home();
    }
  }
    ...
    body: Row(
      children: [
        _PosSideRail(
          ...
          onSecondaryLogOff: _logOffSecondary,
          onHome: () => setState(() => _module = AppModule.home),
          onPos: () => setState(() => _module = AppModule.pos),
          onOrders: () => setState(() => _module = AppModule.orders),
          onFinancials: () => setState(() => _module = AppModule.financials),
          onInventory: () => setState(() => _module = AppModule.inventory),
          onSync: _syncing ? null : _syncRetail,
          onSettings: () =>
              _showInfo('Settings will be expanded in the next module.'),
        ),
