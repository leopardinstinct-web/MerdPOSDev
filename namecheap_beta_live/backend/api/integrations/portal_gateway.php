...
{
    return [
        'beta_state'=>['GET'],'dashboard_data'=>['GET'],'dashboard_layout'=>['GET','POST'],
...
function merd_drupal_gateway_scalar_map(mixed $value, string $field): array
{
    ...
    foreach ($value as $key=>$item) {
        if (!is_string($key) || !preg_match('/^[A-Za-z0-9_.-]{1,64}$/',$key)) throw new MerdRequestException('invalid_request',400,'Invalid ' . $field . '.');
        if (is_array($item)) {
            if (count($item)>100) throw new MerdRequestException('invalid_request',400,'Invalid ' . $field . '.');
            $result[$key]=$item; continue;
        }
        if (!is_scalar($item) && $item !== null) throw new MerdRequestException('invalid_request',400,'Invalid ' . $field . '.');
        $result[$key]=$item;
    }
    return $result;
}
...
try {
    ...
    if (!is_string($raw) || $raw==='' || strlen($raw)>4*1024*1024) throw new MerdRequestException('invalid_request',400,'Invalid request.');
    ...
    if ($contextClientId !== null) {
        ...
        $contextClientId=(int)$validated;
    }
    $routes=merd_drupal_gateway_routes();
    if (!isset($routes[$route]) || !in_array($method,$routes[$route],true)) throw new MerdRequestException('route_not_allowed',403,'This route is not available to Drupal.');
    ...
    if ($isPlatform) {
        ...
        $_SESSION['user']=[
            'id'=>(int)$employee['id'],'identity_scope'=>'employee','client_id'=>(int)$service['client_id'],
            ...
            'role'=>$baseRole,'actual_employee_type'=>$baseRole,'employee_type'=>$baseRole,
            'role_name'=>(string)$role['role_label'],'client_role_id'=>(int)$role['id'],'role_key'=>(string)$role['role_key'],
            'role_label'=>(string)$role['role_label'],'authority_level'=>(int)$role['authority_level'],'is_dev'=>false,
        ];
    ...
    if (!is_file($target)) throw new MerdRequestException('route_not_allowed',403,'This route is not available to Drupal.');
    require $target;
