...
function fk_exists(PDO $pdo, string $constraint): bool
{
    $stmt = $pdo->prepare('SELECT COUNT(*) FROM information_schema.referential_constraints WHERE constraint_schema=DATABASE() AND constraint_name=?');
...
try {
    ...
    if (!fk_exists($pdo, 'fk_employees_client_role')) {
        $pdo->exec('ALTER TABLE employees ADD CONSTRAINT fk_employees_client_role FOREIGN KEY (client_role_id) REFERENCES client_roles(id) ON UPDATE RESTRICT ON DELETE RESTRICT');
    }
    ...
    echo "031 client roles/dashboard templates applied; {$clientCount} clients, {$roleCount} roles, {$mappedEmployees}/{$employeeCount} employees mapped.\n";
} catch (Throwable $e) {
