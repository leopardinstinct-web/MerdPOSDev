...
query_check(str_contains($timesheet,"foreach(\$report['employees'] as &\$employee)")&&str_contains($timesheet,"foreach(\$employee['rows'] as &\$row)")&&!str_contains($timesheet,"foreach((\$report['employees'] ?? []) as &\$employee)"),'Timesheet Query targets must be written into the report arrays, not temporary null-coalesced copies.');
