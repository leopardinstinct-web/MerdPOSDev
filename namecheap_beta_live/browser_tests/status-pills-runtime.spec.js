...
const dashboardCssPath = path.join(portalRoot, 'assets', 'dashboard-builder.css');
const omniPath = path.join(portalRoot, 'assets', 'omnichannel-identity.js');
const fixture = '<!doctype html><html><head></head><body class="merd-shell"><div id="dashboardRolebar" class="dashboard-rolebar"><div><span class="dashboard-rolebar-label">Dashboard role</span></div><div class="dashboard-role-controls"><select id="dashboardRoleSelect"><option>Developer</option></select><button id="dashboardEditToggle" type="button">Edit dashboard</button><button id="dashboardAddButton" type="button">+</button></div></div></body></html>';
...
async function mount(page,{isDev=true,shop=true,width=1280}={}){
  ...
  await page.addStyleTag({path:tokensPath});await page.addStyleTag({path:designSystemPath});await page.addStyleTag({path:dashboardCssPath});await page.addScriptTag({path:omniPath});
}
...
test('DEV dashboard shows current user, preview role and client status pills',async({page})=>{
  ...
  await expect(page.locator('.dashboard-rolebar-label')).toBeHidden();
  await expect(page.locator('#dashboardRoleSelect')).toBeHidden();
  await expect(page.locator('#dashboardEditToggle')).toBeHidden();
  expect(await page.locator('#dashboardStatusPills').evaluate(el => el.parentElement?.classList.contains('dashboard-role-controls'))).toBe(true);
  expect(await page.locator('#dashboardStatusPills').evaluate(el => el.previousElementSibling?.id)).toBe('dashboardEditToggle');
  await expect(page.locator('#omniCurrentUser + #omniPreviewRole')).toHaveCount(1);
