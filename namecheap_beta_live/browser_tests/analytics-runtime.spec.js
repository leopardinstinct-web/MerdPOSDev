...
function dataPayload(storeId=0,days=7){
  const stores=[{id:1,store_name:'Alpha'},{id:2,store_name:'Beta'}];
  ...
  return {
    ...
    filters:{store_id:storeId,days},filter_options:{stores,periods:[7,14,30]},
    ...
    management:{currency_code:'AUD',timezone:'Australia/Sydney',period_days:days,financial_by_store:[],sales_by_store:[],analytics:{sales_period:Array.from({length:days},(_,i)=>({date:`2026-08-${String(i+1).padStart(2,'0')}`,value:i+1})),attendance_period:[],sync_statuses:[]}}
  };
}
...
test('dashboard coordinates store drill-down and reporting-period filters',async({page})=>{
  ...
  await page.route('https://merdpos-smoke.invalid/api/dashboard_data.php*',route=>{
    ...
    const storeId=Number(url.searchParams.get('store_id')||0),days=Number(url.searchParams.get('days')||7);
    requests.push({storeId,days});
    route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(dataPayload(storeId,days))});
  });
  ...
  await page.locator('[data-dashboard-days="14"]').click();
  await expect.poll(()=>requests.at(-1)?.days).toBe(14);
  await expect(page.locator('[data-dashboard-days="14"]')).toHaveAttribute('aria-pressed','true');
...
test('analytics wiring and data endpoint keep filtering inside dashboard authorization scope',async()=>{
  ...
  expect(management.indexOf("assets/analytics-runtime.js?v=20260831analytics2")).toBeLessThan(management.indexOf("assets/dashboard-builder.js?v=20260901ds79"));
  ...
  expect(api).toContain("'filters'=>['store_id'=>$storeId,'days'=>(int)$days]");
  expect(api).toContain("'filter_options'=>['stores'=>$filterStores,'periods'=>[7,14,30]]");
});
