...
async function expectError(response,label,code){ const r=await body(response); const got=r.data?.code||r.data?.error_code||r.data?.error||''; const ok=!r.ok && String(r.text).includes(code); record(label,ok,`${r.status} ${got}`); if(!ok) throw new Error(`${label}: expected ${code}, got ${r.status} ${r.text.slice(0,500)}`); return r; }
