...
test('Color exposes MERDPOS palette first, toggles target, and More colors scrolls in one ring',async({page})=>{
  ...
  await choose(page,'More');await expect(item(page,'Navy')).toHaveCount(0);const menu=page.locator('.merd-ui-menu'),box=await menu.boundingBox(),before=await page.getByRole('menuitem').evaluateAll(nodes=>nodes.map(n=>n.getAttribute('aria-label')));await page.mouse.move(box.x+box.width-12,box.y+box.height/2);await page.mouse.wheel(0,100);const after=await page.getByRole('menuitem').evaluateAll(nodes=>nodes.map(n=>n.getAttribute('aria-label')));expect(after).not.toEqual(before);const swatches=page.locator('.merd-ui-sector-ring.ring-1 [role="menuitem"]');await swatches.first().click();const payload=await page.evaluate(()=>window.MERDPOS_UI_STUDIO.getChangeSet());expect(payload.patches).toEqual(expect.arrayContaining([expect.objectContaining({kind:'style',scope:'matching',property:'background-color'})]));
});

test('Galaxy-sized single ring stays concentric, dark, readable and label-contained',async({page})=>{
  await mount(page,true,360);await page.locator('#openUiStudioBtn').click();await openRoot(page);
  const geometry=await page.evaluate(()=>{const h=document.querySelector('.merd-ui-hub').getBoundingClientRect(),m=document.querySelector('.merd-ui-menu').getBoundingClientRect(),e=document.querySelector('[aria-label="Select"]'),p=e?.querySelector('.merd-ui-sector'),l=e?.querySelector('.merd-ui-sector-label'),i=e?.querySelector('.merd-ui-sector-icon');const pb=p?.getBoundingClientRect(),lb=l?.getBoundingClientRect(),ib=i?.getBoundingClientRect();return {dx:Math.abs((h.left+h.width/2)-(m.left+m.width/2)),dy:Math.abs((h.top+h.height/2)-(m.top+m.height/2)),fill:p?.getAttribute('fill'),labelH:lb?.height||0,iconW:ib?.width||0,labelInside:!!pb&&!!lb&&lb.left>=pb.left-1&&lb.right<=pb.right+1&&lb.top>=pb.top-1&&lb.bottom<=pb.bottom+1}});expect(geometry.dx).toBeLessThan(1);expect(geometry.dy).toBeLessThan(1);expect(geometry.fill).toBe('#25253D');expect(geometry.labelInside).toBe(true);expect(geometry.labelH).toBeGreaterThanOrEqual(16);expect(geometry.iconW).toBeGreaterThanOrEqual(29);
  const labelBox=await item(page,'Edit').locator('.merd-ui-sector-label').boundingBox();await page.mouse.click(labelBox.x+labelBox.width/2,labelBox.y+labelBox.height/2);await expect(item(page,'Color')).toBeVisible();await expect(item(page,'Select')).toHaveCount(0);const fit=await page.evaluate(()=>{const e=document.querySelector('[aria-label="Hide"]'),p=e?.querySelector('.merd-ui-sector'),l=e?.querySelector('.merd-ui-sector-label'),i=e?.querySelector('.merd-ui-sector-icon');if(!p||!l||!i)return null;const pb=p.getBoundingClientRect(),lb=l.getBoundingClientRect(),ib=i.getBoundingClientRect();return {labelInside:lb.left>=pb.left-1&&lb.right<=pb.right+1&&lb.top>=pb.top-1&&lb.bottom<=pb.bottom+1,labelH:lb.height,iconW:ib.width}});expect(fit).not.toBeNull();expect(fit.labelInside).toBe(true);expect(fit.labelH).toBeGreaterThanOrEqual(17);expect(fit.iconW).toBeGreaterThanOrEqual(29);
  await page.setViewportSize({width:681,height:598});await page.waitForTimeout(80);const resized=await page.evaluate(()=>{const h=document.querySelector('.merd-ui-hub').getBoundingClientRect(),m=document.querySelector('.merd-ui-menu').getBoundingClientRect(),label=document.querySelector('[aria-label="Color"] .merd-ui-sector-label')?.getBoundingClientRect();return {dx:Math.abs((h.left+h.width/2)-(m.left+m.width/2)),dy:Math.abs((h.top+h.height/2)-(m.top+m.height/2)),menu:m.width,labelH:label?.height||0}});expect(resized.dx).toBeLessThan(1);expect(resized.dy).toBeLessThan(1);expect(resized.menu).toBeGreaterThan(380);expect(resized.labelH).toBeGreaterThanOrEqual(18);
});

test('numeric layout values use a two-slice stepper and show current value in the hub',async({page})=>{
  await mount(page,true);await page.locator('#openUiStudioBtn').click();await selectTarget(page,'#cardA');await openEdit(page);await choose(page,'Layout');await choose(page,'Padding');await expect(page.getByRole('menuitem')).toHaveCount(2);await expect(item(page,'Increase')).toBeVisible();await expect(item(page,'Decrease')).toBeVisible();await expect(hub(page).locator('small')).toContainText('PADDING');const before=await hub(page).locator('strong').textContent();await choose(page,'Increase');const after=await hub(page).locator('strong').textContent();expect(after).not.toBe(before);const payload=await page.evaluate(()=>window.MERDPOS_UI_STUDIO.getChangeSet());expect(payload.patches).toEqual(expect.arrayContaining([expect.objectContaining({kind:'style',property:'padding'})]));
});
