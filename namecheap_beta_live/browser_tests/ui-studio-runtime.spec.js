...
<dialog id="testModal"><div style="width:28rem;height:18rem;padding:2rem;background:#fff">Test modal</div></dialog>
...
test('changed element dot follows target page visibility and modal occlusion',async({page})=>{
  const server=await mount(page,true);await selectTarget(page,'#cardA');await choose(page,'Comment');await submitComment(page,'Unresolved marker');await expect.poll(()=>server.state.patches.length).toBe(1);await page.mouse.click(5,5);const dot=page.locator('.merd-ui-change-dot');await expect(dot).toHaveCount(1);await expect(dot).toBeVisible();await expect(page.locator('[data-ui-studio="history"],.merd-ui-element-history')).toHaveCount(0);await expect(dot).toHaveAttribute('aria-label','Unresolved Studio patch');await page.locator('.portal-tab[data-panel="secondPanel"]').click();await expect(dot).toBeHidden();await page.locator('.portal-tab[data-panel="demoPanel"]').click();await expect(dot).toBeVisible();await page.evaluate(()=>document.getElementById('testModal').showModal());await expect(dot).toBeHidden();await page.evaluate(()=>document.getElementById('testModal').close());await expect(dot).toBeVisible();
});

test('DevStudio radial remains the top browser layer after a MERDPOS modal opens',async({page})=>{
  await mount(page,true);await selectTarget(page,'#cardA');await expect(item(page,'Unselect')).toBeVisible();await page.evaluate(()=>document.getElementById('testModal').showModal());await expect(page.locator('#testModal')).toHaveJSProperty('open',true);await expect.poll(()=>page.evaluate(()=>document.querySelector('.merd-ui-studio-host').parentElement?.id)).toBe('testModal');await expect(page.locator('.merd-ui-studio-host')).toHaveAttribute('data-modal-host','1');const topHit=await page.evaluate(()=>{const hub=document.querySelector('.merd-ui-hub').getBoundingClientRect(),node=document.elementFromPoint(hub.left+hub.width/2,hub.top+hub.height/2);return !!node?.closest?.('.merd-ui-studio-host');});expect(topHit).toBe(true);await page.mouse.move(640,320);const pill=page.locator('.merd-ui-cursor-pill');await expect(pill).toBeVisible();const pillTopHit=await page.evaluate(()=>{const pill=document.querySelector('.merd-ui-cursor-pill'),prior=pill.style.pointerEvents,r=pill.getBoundingClientRect();pill.style.pointerEvents='auto';const node=document.elementFromPoint(r.left+r.width/2,r.top+r.height/2);pill.style.pointerEvents=prior;return node===pill||!!node?.closest?.('.merd-ui-cursor-pill');});expect(pillTopHit).toBe(true);await expect(item(page,'Unselect')).toBeVisible();await page.evaluate(()=>document.getElementById('testModal').close());await expect.poll(()=>page.evaluate(()=>document.querySelector('.merd-ui-studio-host').parentElement===document.body)).toBe(true);await expect.poll(()=>page.evaluate(()=>document.querySelector('.merd-ui-studio-host').matches(':popover-open'))).toBe(true);
});
