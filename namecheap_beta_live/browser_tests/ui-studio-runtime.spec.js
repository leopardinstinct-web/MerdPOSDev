...
async function mount(page,isDev=true,width=1280,enabled=true){
...
async function openRoot(page){for(let i=0;i<6;i++){if(await item(page,'Settings').isVisible().catch(()=>false))return;await hub(page).click();await page.waitForTimeout(20)}await expect(item(page,'Settings')).toBeVisible()}
async function selectTarget(page,selector){const target=page.locator(selector);await target.dispatchEvent('mouseover');await expect(page.locator('.merd-ui-hover-select')).toBeVisible();await page.locator('.merd-ui-hover-select').click();await expect(page.locator('.merd-ui-studio-selected')).toHaveCount(1)}
...
test('DEV Studio is toggle-controlled and root no longer has Select',async({page})=>{
  await mount(page,true,1280,false);await expect(hub(page)).toBeHidden();expect(await page.evaluate(()=>window.MERDPOS_UI_STUDIO.isEnabled())).toBe(false);await page.evaluate(()=>window.MERDPOS_UI_STUDIO.setEnabled(true));await expect(hub(page)).toBeVisible();await openRoot(page);await expect(item(page,'Minimize')).toBeVisible();await expect(item(page,'Select')).toHaveCount(0);await expect(item(page,'Edit Dashboard')).toBeVisible();await expect(item(page,'Settings')).toBeVisible();await expect(item(page,'Exit')).toHaveCount(0);await expect(item(page,'Undo')).toBeVisible();
});
...
test('hover Select can replace a selection and selected root exposes Unselect',async({page})=>{
...
test('hub hover opens and wheel arms enabled root actions on fine pointers',async({page})=>{
  await mount(page,true,1280);const box=await hub(page).boundingBox();await page.mouse.move(box.x+box.width/2,box.y+box.height/2);await expect(item(page,'Settings')).toBeVisible();await page.mouse.wheel(0,120);await expect(hub(page).locator('strong')).toHaveText('Minimize');await page.mouse.wheel(0,120);await expect(hub(page).locator('strong')).toHaveText('Edit Dashboard');await hub(page).click();await expect.poll(()=>page.evaluate(()=>window.__dashboardEditCalls)).toBe(1);
});
...
test('hover Select chooses existing controls without activating them',async({page})=>{
