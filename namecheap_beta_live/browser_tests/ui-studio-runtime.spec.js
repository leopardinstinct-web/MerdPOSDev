...
test('Galaxy-sized touch layout keeps hub concentric and sector content readable',async({page})=>{
  ...
  expect(Math.abs(geometry.hub.x-geometry.menu.x)).toBeLessThan(1);expect(Math.abs(geometry.hub.y-geometry.menu.y)).toBeLessThan(1);expect(geometry.label.h).toBeGreaterThanOrEqual(12);expect(geometry.icon.w).toBeGreaterThanOrEqual(18);
  const labelBox=await item(page,'Edit').locator('.merd-ui-sector-label').boundingBox();await page.mouse.click(labelBox.x+labelBox.width/2,labelBox.y+labelBox.height/2);await expect(item(page,'Color')).toBeVisible();
