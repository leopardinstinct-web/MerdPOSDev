...
function createStudioServer(){
  ...
  const payload=()=>({success:true,csrf:'studio-csrf',revision:state.revision,patches:structuredClone(state.patches),history:[...state.history].reverse(),generated_at:new Date().toISOString()});
  return {state,async handle(route){const request=route.request();if(request.method()==='GET')return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(payload())});const body=request.postDataJSON()||{};if(body.action!=='bootstrap'&&Number(body.base_revision)!==state.revision){return route.fulfill({status:409,contentType:'application/json',body:JSON.stringify({...payload(),success:false,error_code:'revision_conflict',error:'Studio changed elsewhere'})});}if(body.action==='bootstrap'){if(state.revision===0&&!state.history.length){state.patches=structuredClone(body.patches||[]);for(const entry of body.history||[])state.history.push({...entry,id:`legacy-${state.nextId++}`,actor:'Developer',roleScope:entry.roleScope||'DEV'});state.revision=1;}return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(payload())});}if(body.action==='mutate'){state.patches=structuredClone(body.patches||[]);state.revision++;state.history.push({...body.entry,id:`global-${state.nextId++}`,actor:'Developer',roleScope:body.entry?.roleScope||'DEV',revision:state.revision,pending:false});return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(payload())});}if(body.action==='delete'){state.history=state.history.filter(entry=>entry.id!==body.history_id);state.patches=[];state.revision++;return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(payload())});}return route.fulfill({status:400,contentType:'application/json',body:JSON.stringify({success:false,error:'Bad Studio action'})});}};
}
...
test('Changes restores global History and JSON controls',async({page})=>{
  await mount(page,true,1280);await openRoot(page);await expect(item(page,'Changes')).toBeVisible();await choose(page,'Changes');await expect(item(page,'History')).toBeVisible();await expect(item(page,'Copy')).toBeVisible();await expect(item(page,'Chat')).toBeVisible();await expect(item(page,'Undo')).toBeVisible();await hub(page).click({button:'right'});await selectTarget(page,'#cardA');await choose(page,'Hide');await expect.poll(()=>page.evaluate(()=>window.MERDPOS_UI_STUDIO.getHistory().some(entry=>!entry.pending))).toBe(true);await choose(page,'Changes');await choose(page,'Copy');const copied=JSON.parse(await page.evaluate(()=>window.__studioCopied));expect(copied.version).toBe(3);expect(copied.global).toBe(true);expect(copied.patches.length).toBeGreaterThan(0);
});

test('changed element dot opens floating global history with trash action',async({page})=>{
  await mount(page,true);await selectTarget(page,'#cardA');page.once('dialog',d=>d.accept('Global history marker'));await choose(page,'Comment');await expect.poll(()=>page.evaluate(()=>window.MERDPOS_UI_STUDIO.getHistory().some(entry=>!entry.pending))).toBe(true);await page.mouse.click(5,5);const dot=page.locator('.merd-ui-change-dot');await expect(dot).toHaveCount(1);await expect(dot).toBeVisible();await dot.hover();const floating=page.locator('.merd-ui-element-history');await expect(floating).toBeVisible();await expect(floating).toContainText('Global history marker');await expect(floating.locator('.merd-ui-history-delete img')).toHaveCount(1);const target=await page.locator('#cardA').boundingBox(),panel=await floating.boundingBox();expect(panel.x+panel.width<=target.x||panel.x>=target.x+target.width||panel.y+panel.height<=target.y||panel.y>=target.y+target.height).toBe(true);
});
...
test('change-count badge opens History and a history step can be deleted',async({page})=>{
  await mount(page,true);await selectTarget(page,'#cardA');await choose(page,'Hide');await expect(page.locator('.merd-ui-hub-count')).toBeVisible();const before=await page.evaluate(()=>window.MERDPOS_UI_STUDIO.getHistory().length);await page.locator('.merd-ui-hub-count').click();const history=page.locator('[data-ui-studio="history"]');await expect(history).toBeVisible();await expect(history.locator('.merd-ui-history-delete').first()).toBeVisible();await history.locator('.merd-ui-history-delete').first().click();await expect.poll(()=>page.evaluate(()=>window.MERDPOS_UI_STUDIO.getHistory().length)).toBe(before-1);
});
...
test('global Studio history is shared across independent Developer browser contexts',async({browser})=>{
  const server=createStudioServer(),contextA=await browser.newContext(),contextB=await browser.newContext(),pageA=await contextA.newPage(),pageB=await contextB.newPage();try{await mount(pageA,true,1280,true,server);await mount(pageB,true,1280,true,server);await selectTarget(pageA,'#cardA');await choose(pageA,'Hide');await expect.poll(()=>server.state.history.length).toBe(1);await pageB.waitForTimeout(4300);await reloadRuntime(pageB);await expect.poll(()=>pageB.evaluate(()=>window.MERDPOS_UI_STUDIO?.getHistory().length||0)).toBe(1);expect(await pageB.evaluate(()=>window.MERDPOS_UI_STUDIO.getChangeSet().revision)).toBe(server.state.revision);await expect(pageB.locator('#cardA')).toBeHidden();}finally{await contextA.close();await contextB.close();}
});
