(function () {
  ...
  function hideLegacyStoreFields() {
    ...
    if (employeeDescription) employeeDescription.textContent = 'Accounts, store access, role, Level of Authority, pay rate and effective dates.';
  }
  ...
  function renderEmployees(filter = '') {
    ...
    employeeRoot.innerHTML = rows.map(employee => {
      ...
      const nextRate = employee.next_rate ? `<div class="entity-sub rate-future">Next rate ${money(employee.next_rate.hourly_rate)} from ${esc(formatDate(employee.next_rate.effective_from))}</div>` : '';
      return `<article class="entity-row ${employee.status === 'inactive' ? 'is-muted' : ''}">
        ...
        <div class="entity-meta">${rolePill(employee)}<span class="store-access-summary">LOA ${Number(employee.role_authority || 0)}</span>${storeAccessPill(employee)}<span class="entity-rate">${money(employee.hourly_rate)}/hr</span>${statusPill(employee.status)}</div>
  ...
  function renderStores(filter = '') {
    ...
    storeRoot.innerHTML = rows.map(store => `<article class="entity-row ${store.status==='inactive'?'is-muted':''}"><div class="entity-avatar store-avatar">${icon('store')}</div><div class="entity-copy"><div class="entity-title-line"><strong>${esc(store.store_name)}</strong></div></div><div class="entity-meta">${statusPill(store.status)}</div><button type="button" class="icon-text-btn" data-edit-store="${Number(store.id)}">${icon('edit')}<span>Edit</span></button></article>`).join('');
  ...
  function openEmployee(id = null) {
    ...
    if (roleSelect) roleSelect.disabled = !!employee?.self;
  ...
  async function saveForm(form, action) {
    ...
    values.action = action; values.csrf = directory.csrf;
    try {
      const button = form.querySelector('[type="submit"]'); button.disabled = true;
      directory = await api('api/admin_directory.php', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(values)});
  ...
  ensureStylesheet(); ensureStoreAccessControls(); ensureRateEffectiveField(); hideLegacyStoreFields(); ensureTimingsModule();
