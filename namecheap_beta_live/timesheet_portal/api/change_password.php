...
try {
    ...
    if (!preg_match('/^\d{6,20}$/', $new)) throw new MerdWorkforceException('invalid_password', 'New password must contain 6–20 digits.');
    ...
    if (beta_user_is_platform_identity($user)) {
        $identityId = beta_actor_platform_identity_id($user);
        if ($identityId === null) throw new MerdWorkforceException('account_inactive', 'Platform identity is unavailable.');
        $pdo->beginTransaction();
        try {
            $stmt = $pdo->prepare("SELECT id,login_password,pin_code FROM platform_identities WHERE id=? AND role_key='DEV' AND status='active' FOR UPDATE");
            $stmt->execute([$identityId]);
            $identity = $stmt->fetch(PDO::FETCH_ASSOC);
            if (!is_array($identity) || !merd_employee_authenticates($identity, $current)) {
                throw new MerdWorkforceException('invalid_current_password', 'Current password is incorrect.');
            }
            $hash = password_hash($new, PASSWORD_DEFAULT);
            $pdo->prepare('UPDATE platform_identities SET login_password=?,pin_code=? WHERE id=?')->execute([$hash,$hash,$identityId]);
            try {
                $audit = $pdo->prepare(
                    "INSERT INTO security_audit_events (client_id,employee_id,platform_identity_id,event_type,outcome,actor_type,actor_id,ip_address,user_agent,metadata) "
                    . "VALUES (?,NULL,?,'portal_password_change','success','platform',?,?,?,?)"
                );
                $audit->execute([(int)$user['client_id'],$identityId,(string)$user['user_id'],
        ...
        $pdo->beginTransaction();
        try {
            $stmt = $pdo->prepare("SELECT id,client_id,login_password,pin_code FROM employees WHERE id=? AND client_id=? AND status='active' FOR UPDATE");
            ...
            $hash = password_hash($new, PASSWORD_DEFAULT);
            $pdo->prepare('UPDATE employees SET login_password=?,pin_code=? WHERE id=? AND client_id=?')->execute([$hash,$hash,(int)$user['id'],$authClientId]);
            try {
                $audit = $pdo->prepare(
