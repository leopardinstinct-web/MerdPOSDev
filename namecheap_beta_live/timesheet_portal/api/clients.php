...
function clients_code(mixed $value): string
{
    ...
    if (strlen($code) < 2 || strlen($code) > 50) {
        throw new MerdWorkforceException('invalid_client_code', 'Client Code must be 2–50 characters.');
    }
    if (!preg_match('/^[A-Z0-9][A-Z0-9_-]{1,49}$/', $code)) {
        throw new MerdWorkforceException('invalid_client_code', 'Client Code must start with a letter or number and use only A–Z, 0–9, hyphen or underscore.');
    }
    if (in_array($code, ['ALL','NONE','NULL','SYSTEM','ADMIN','SUPER','DEV','CLIENT'], true)) {
        throw new MerdWorkforceException('reserved_client_code', 'That Client Code is reserved. Choose a more specific identifier.');
    }
    return $code;
}
...
function clients_name(mixed $value): string
{
    ...
    if ($name === '' || mb_strlen($name) > 100) {
        throw new MerdWorkforceException('invalid_client_name', 'Enter a client name up to 100 characters.');
    }
    return $name;
}
...
try {
    ...
    $input = request_input();
    require_csrf($input);
    if ((string)($input['action'] ?? '') !== 'save_client') {
    ...
    if ($dupCode->fetchColumn()) {
        throw new MerdWorkforceException('duplicate_client_code', 'That Client Code is already assigned to another client.');
    }
    ...
    if ($dupName->fetchColumn()) {
        throw new MerdWorkforceException('duplicate_client_name', 'A client with that name already exists.');
    }
    ...
    try {
        ...
        if ((string)$e->getCode() === '23000') {
            throw new MerdWorkforceException('duplicate_client', 'Client name or Client Code conflicts with an existing client.');
        }
        throw $e;
    } catch (Throwable $e) {
        if ($pdo->inTransaction()) $pdo->rollBack();
    ...
    clients_audit($pdo, $user, (int)$id, $action, [
        'name' => $name,
        'client_code' => $code,
        'status' => $status,
    ]);
