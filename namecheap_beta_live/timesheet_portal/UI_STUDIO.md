...
- With no selection, the root offers **Select** plus shared **Undo**. Once one element is selected, the root becomes **Select**, **Add**, **Edit**, **Move**, **Comment**, **Hide/Show**, plus **Undo**. Select always replaces the prior selection, so only one element is selected at a time. There is no Exit wedge.
