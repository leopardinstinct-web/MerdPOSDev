...
- The root ring exposes **Select**, **Edit**, **Changes**, **Exit** and the shared **Undo** action. Undo is appended to every radial level and is disabled when there is nothing to undo.
...
- **Move** exposes Before, After and Inside and remains This-element-only. When a selected/destination node is inside a recognized MERDPOS component, Move promotes it to that component root so moving a card/feature does not accidentally move only its inner text/icon.
