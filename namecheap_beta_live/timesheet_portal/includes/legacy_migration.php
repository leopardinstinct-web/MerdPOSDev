...
function legacy_save_sources(PDO $pdo, array $actor, int $clientId, array $input): void
{
    ...
    try {
        $upsert = $pdo->prepare(
            'INSERT INTO client_legacy_sources (client_id,source_type,provider,spreadsheet_id,sheet_names_json,status,created_by_employee_id,created_by_platform_identity_id,updated_by_employee_id,updated_by_platform_identity_id) '
            . "VALUES (?,?,'google_public_csv',?,?,'active',?,?,?,?) "
            . "ON DUPLICATE KEY UPDATE spreadsheet_id=VALUES(spreadsheet_id),sheet_names_json=VALUES(sheet_names_json),status='active',updated_by_employee_id=VALUES(updated_by_employee_id),updated_by_platform_identity_id=VALUES(updated_by_platform_identity_id)"
        );
        $upsert->execute([$clientId,'attendance',$attendanceId,json_encode($attendance,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR),beta_actor_employee_id($actor),beta_actor_platform_identity_id($actor),beta_actor_employee_id($actor),beta_actor_platform_identity_id($actor)]);
        if ($financialId !== null) {
            $upsert->execute([$clientId,'financial',$financialId,json_encode($financial,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR),beta_actor_employee_id($actor),beta_actor_platform_identity_id($actor),beta_actor_employee_id($actor),beta_actor_platform_identity_id($actor)]);
        } else {
            $pdo->prepare("UPDATE client_legacy_sources SET status='inactive',updated_by_employee_id=?,updated_by_platform_identity_id=? WHERE client_id=? AND source_type='financial'")->execute([beta_actor_employee_id($actor),beta_actor_platform_identity_id($actor),$clientId]);
        }
        $pdo->prepare('INSERT IGNORE INTO client_migration_state (client_id) VALUES (?)')->execute([$clientId]);
        $pdo->commit();
    } catch (Throwable $e) {
...
function legacy_run_batch(PDO $pdo,array $actor,int $clientId,string $mode): array
{
    ...
    @set_time_limit(240);
    $public=merd_uuid_v4();$stmt=$pdo->prepare("INSERT INTO legacy_migration_batches (public_id,client_id,mode,status,started_by_employee_id,started_by_platform_identity_id) VALUES (?,?,?,'running',?,?)");$stmt->execute([$public,$clientId,$mode,beta_actor_employee_id($actor),beta_actor_platform_identity_id($actor)]);$batchId=(int)$pdo->lastInsertId();
...
{
    try { $copy=$actor; $copy['client_id']=$clientId; beta_admin_audit($pdo,$copy,$action,'legacy_migration',(string)$clientId,$details); }
    catch(Throwable $e){ error_log('MERDPOS legacy migration audit failed: '.get_class($e)); }
}
