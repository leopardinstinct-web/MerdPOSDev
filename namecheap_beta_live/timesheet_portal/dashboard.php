...
<head>
  ...
  <title>MerdPOS Workforce Beta</title>
...
<body>
  ...
  <main class="page-shell">
    <nav class="portal-tabs" aria-label="Portal sections">
      <button class="portal-tab active" data-panel="dashboardPanel">Dashboard</button>
    ...
    <section id="dashboardPanel" class="portal-panel">
      <div class="panel-heading"><div><h1>Dashboard</h1><p class="muted"><?= $user['is_super'] ? 'Team overview, pending approval work, and live QR attendance.' : 'Your current shift, store access, and recent activity.' ?></p></div><button id="refreshBetaBtn" class="secondary-btn">Refresh</button></div>
    ...
    <section id="disputesPanel" class="portal-panel" hidden>
      <div class="panel-heading"><div><h1><?= $user['is_super'] ? 'Dispute approvals' : 'Fix a shift' ?></h1><p class="muted"><?= $user['is_super'] ? 'Review pending, approved, rejected, and cancelled disputes raised by USER accounts.' : 'Raise a shift correction for SUPER approval. Nothing changes until it is approved.' ?></p></div></div>
    ...
    <section id="financialPanel" class="portal-panel" hidden>
      <div class="panel-heading"><div><h1>Store financials</h1><p class="muted"><?= $user['is_super'] ? 'SUPER can open days and review all store financials.' : 'USER access is limited to the store where you are currently clocked in by QR.' ?></p></div><span id="financialQueue" class="queue-pill">0 pending</span></div>
      <section class="controls-card">
